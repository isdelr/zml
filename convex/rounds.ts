...
// Admin/owner tool: Recompute finalized voter counter for a round and reconcile the sharded counter
export const recountVoterCounterForRound = mutation({
  args: { roundId: v.id("rounds") },
  handler: async (ctx, args) => {
    const round = await ctx.db.get(args.roundId);
    if (!round) throw new Error("Round not found");
    ...
    // Aggregate votes by user in this round
    ...
    for (const { up, down } of sums.values()) {
      if (up === maxUp && down === maxDown) finalizedCount++;
    }

    const currentCount = await voterCounter.count(ctx, round._id);
    const diff = finalizedCount - currentCount;
    ...
    if (diff > 0) {
      ...
      for (let i = 0; i < -diff; i++) {
        await voterCounter.dec(ctx, round._id);
      }
    }

    const after = await voterCounter.count(ctx, round._id);

    return { before: currentCount, after, adjustedBy: diff };
  },
});
