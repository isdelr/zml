...
export const _seedInternal = internalMutation({
  ...
  handler: async (ctx, { adminUserId, namespace, userCount, prefix }) => {
    const now = Date.now();
    ...
    const createRound = async (
      ...
      const roundId = await ctx.db.insert("rounds", { ...base, ...(cfg as any) });
      return roundId;
    };
    ...
    const leagueDoc = await ctx.db.get(leagueFinished);
    ...
    for (const league of leagues) {
      ...
      for (const round of rounds) {
        ...
        const votes = await ctx.db
          ...
          .withIndex("by_round_and_user", (q) => q.eq("roundId", round._id))
          .collect();
        for (const vte of votes) await ctx.db.delete(vte._id);

        // Listen progress (scan and filter by submissionId)
        const allProgress = await ctx.db.query("listenProgress").collect();
        for (const p of allProgress) {
          if (subIds.has(p.submissionId.toString())) await ctx.db.delete(p._id);
        }

        // Comments
        const comments = await ctx.db
          .query("comments")
          .withIndex("by_submission", (q) => q.eq("submissionId", subs[0]?.
            _id ?? ("000000000000000000000000" as any)))
          .collect();
        // Above index search needs a specific submission; fallback to scan if none
        if (subs.length > 0) {
          for (const s of subs) {
          ...
          for (const c of scan) {
            if (subIds.has(c.submissionId.toString())) await ctx.db.delete(c._id);
          }
        }
